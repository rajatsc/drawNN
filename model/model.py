...
class mySimpleModel2(BaseModel):
	def __init__ (self, input_size=28, num_classes = 25):
		super().__init__()
		self.num_classes = num_classes

		self.conv1 = nn.Sequential(
			nn.Conv2d(in_channels=1, out_channels=8, kernel_size=5, stride=1, padding=2, dilation=1, groups=1, bias=False), 
			nn.ReLU(inplace=True), 
			nn.MaxPool2d(2, 2)
			)

		self.conv2 = nn.Sequential(
			nn.Conv2d(in_channels=8, out_channels=16, kernel_size=5, stride=1, padding=2, dilation=1, groups=1, bias=False),
			nn.ReLU(inplace=True), 
			nn.MaxPool2d(2, 2)
			)
